...
# Load all datasets
item_pairs_1 <- read.csv("data/item_pairs_1.csv")
item_ref <- read.csv("data/item_ref_1.csv")
item_pairs <- item_pairs_1  
trait_description <- read.csv('data/mbt_category.csv')
personality_description <- read.csv('data/personality_desc.csv')
...
shinyServer(function(input, output, session) {
    ...
    personalityOutput <- reactive({
        Section1 <- calculateScore(input$s11,input$s12,input$s13,input$s14,"E","I")
        Section2 <- calculateScore(input$s21,input$s22,input$s23,input$s24,"S","N")
        Section3 <- calculateScore(input$s31,input$s32,input$s33,input$s34,"T","F")
        Section4 <- calculateScore(input$s41,input$s42,input$s43,input$s44,"J","P")
        test <-  as.character(c(Section1, Section2, Section3, Section4))
        test_str <- paste(test, collapse = '')
        print(test_str)
        
        # map personality
        if (Section1 == 'E') { str_1 = 'EXTRAVERT (E)' } else { str_1 = 'INTROVERT (I)' }
        if (Section2 == 'S') { str_2 = 'SENSOR (S)' } else { str_2 = 'INTUITIVE (N)' }
        if (Section3 == 'T') { str_3 = 'THINKERS (T)'} else { str_3 = 'FEELERS (F)'}
        if (Section4 == 'J') { str_4 = 'JUDGES (J)'} else { str_4 = 'PERCEIVERS (P)'}
        
        desc_1 <- 'Extraverts mendapat tenaga daripada berinteraksi dengan orang lain, manakala introvert mendapat tenaga dari dalam diri mereka sendiri. Extraversion termasuk sifat-sifat bertenaga, suka bercakap, dan tegas.
        Mereka gembira bersama dengan orang-orang, yang mengambil bahagian dalam perhimpunan sosial, dan penuh dengan tenaga.'
        desc_2 <- "Orang yang suka belajar perkara-perkara baru dan menikmati pengalaman baru biasanya skor tinggi dalam keterbukaan. Keterbukaan termasuk ciri-ciri seperti yang berwawasan dan mempunyai daya imaginasi 
        dan mempunyai pelbagai kepentingan."
        desc_3 <- "Seseorang dengan tahap yang tinggi agreeableness dalam ujian personaliti biasanya hangat, mesra, dan bijaksana. Mereka biasanya mempunyai pandangan optimistik sifat manusia dan bergaul dengan baik 
        dengan orang lain."
        desc_4 <- "Dimensi ini berkaitan dengan kestabilan emosi seseorang dan tahap emosi negatif. Orang yang menunjukkan prestasi tinggi dengan stres sering mengalami ketidakstabilan emosi dan emosi negatif. 
        Ciri-ciri termasuk makhluk muram dan tegang. Seseorang yang kaya dengan stres kerja mempunyai kecenderungan untuk mudah emosi pengalaman negatif."
        
        data.frame(
            Kategori = c("Bahagian 1","Bahagian 2", "Bahagian 3", "Bahagian 4"),
            Personaliti = as.character(c(str_1, str_2, str_3, str_4)),
            #Intrepetasi = as.character(c(desc_1, desc_2, desc_4)),
            stringsAsFactors = FALSE)
    })
    
    intepretationOutput <- reactive({
        as.character(personalityOutput %>% select(c(personalityValue())))
    })
    
    
    # Helper function
    personalityValue <- reactive({
        Section1 <- calculateScore(input$s11,input$s12,input$s13,input$s14,"E","I")
        Section2 <- calculateScore(input$s21,input$s22,input$s23,input$s24,"S","N")
        Section3 <- calculateScore(input$s31,input$s32,input$s33,input$s34,"T","F")
        Section4 <- calculateScore(input$s41,input$s42,input$s43,input$s44,"J","P")
    ...
    observeEvent(input$submit_2, {
        ...
        output$first <- renderText({
            as.character(c(personalityValue(), " ialah seorang ", personality_description %>% select(personalityValue())))
            
        })
        
        output$second <- renderTable({
            trait_description %>% select(c('Kerjaya', personalityValue()))
        })
    })
    ...
    output$table <- DT::renderDataTable(DT::datatable({
        data <- vacancy
        if (input$pos != "All") {
            data <- data[data$position == input$pos,]
        }
        if (input$edu != "All") {
            data <- data[data$education == input$edu,]
        }
        if (input$state != "All") {
    ...
    observeEvent( input$btn1, {
        if ( input$item_name == "" ) {
            showModal(modalDialog(title = "Pick a starting job first.",
                                  "Sila pilih salah satu kerjaya daripada menu dibawah untuk mulakan proses ini",
    ...
    observeEvent( input$goBack, {
        ...
        if( values$data == 5 & !is.null(input$select5_rows_selected) ) {
            showModal(
                modalDialog("Sila buang pilihan anda sebelum undur kebelakang.", size = "s",
                            title = "Oops!",
    ...
    output$select1 <- renderUI({
        selectizeInput("item_name", label = "",
                       ...
                       width = "100%",
                       options = list(
                           placeholder = 'Mulakan pilihan anda dengan memilih satu kerjaya dibawah.',
    ...
    output$select2 <- DT::renderDataTable({
        datatable( top1(), escape = FALSE,
            ...
            formatCurrency('SalaryDiff', 'RM') %>% 
            formatPercentage('Prob', 1) %>%
            formatCurrency("Salary2Min", 'RM')
    })
    ...
    output$stepNo <- renderUI({
        if(values$data == 1) {
            ...
            # tags$h4("Step 2:")
            div(tags$h4("Step 2:"), div(tags$h6("Pilih satu daripada kerjaya-kerjaya di bawah")))
        } else if (values$data == 3) {
            tags$h4("Step 3:")
        } else if (values$data == 4) {
            tags$h4("Step 4:")
            ...
            div(class="panel panel-default",
                div(class="panel-body",
                    div(tags$img(src = "one.svg", width = "25px", height = "25px"), tags$h6( paste0(input$item_name, " (", job_1_data()[2], ")") ),
                        paste0( job_1_data()[3], " - ", job_1_data()[4], " /bulan"), 
                        div(paste0(job_1_data()[5], " penyandang"))
                    )
                ))
        }
    })
    ...
    label_1 <- reactive({
        ...
        lab <- paste0( input$item_name, "\n",
                       job_1_data()[3], " - ", job_1_data()[4], " Bulanan",
                       " | ", job_1_data()[5], " Kekosongan")
            ...
            div(class="panel panel-default",
                div(class="panel-body",
                    div(tags$img(src = "two.svg", width = "25px", height = "25px"), tags$h6( paste0(job_2_data()[1], " (", job_2_data()[2], ")") ),
                        paste0( job_2_data()[3], " - ", job_2_data()[4], " /bulan"), 
                        div(paste0(job_2_data()[5], " penyandang"))
                    )
                ))
        }
    })
    ...
    label_2 <- reactive({
        ...
        try(
            paste0( job_2_data()[1], "\n",
                    job_2_data()[3], " - ", job_2_data()[4], " Bulanan", "\n",
                    job_2_data()[6], " Popularity", " | ", job_2_data()[5], " Kekosongan"),
            ...
            div(class="panel panel-default",
                div(class="panel-body",
                    div(tags$img(src = "three.svg", width = "25px", height = "25px"), tags$h6( paste0(job_3_data()[1], " (", job_3_data()[2], ")") ),
                        paste0( job_3_data()[3], " - ", job_3_data()[4], " /bulan"), 
                        div(paste0(job_3_data()[5], " penyandang"))
                    )
                ))
        }
    })
    ...
    label_3 <- reactive({
        ...
        try(
            paste0( job_3_data()[1], "\n",
                    job_3_data()[3], " - ", job_3_data()[4], " Bulanan", "\n",
                    job_3_data()[6], " Popularity", " | ", job_3_data()[5], " Kekosongan"),
            TRUE
        )
            ...
            div(class="panel panel-default",
                div(class="panel-body",
                    div(tags$img(src = "four.svg", width = "25px", height = "25px"), tags$h6( paste0(job_4_data()[1], " (", job_4_data()[2], ")") ),
                        paste0( job_4_data()[3], " - ", job_4_data()[4], " /bulan"), 
                        div(paste0(job_4_data()[5], " penyandang"))
                    )
                ))
        }
    })
    ...
    label_4 <- reactive({
        try(
            paste0( job_4_data()[1], "\n",
                    job_4_data()[3], " - ", job_4_data()[4], " Bulanan", "\n",
                    job_4_data()[6], " Popularity", " | ", job_4_data()[5], " Kekosongan"),
            TRUE
        )
            ...
            div(class="panel panel-default",
                div(class="panel-body",
                    div(tags$img(src = "five.svg", width = "25px", height = "25px"), tags$h6( paste0(job_5_data()[1], " (", job_5_data()[2], ")") ),
                        paste0( job_5_data()[3], " - ", job_5_data()[4], " /bulan"), 
                        div(paste0(job_5_data()[5], " penyandang"))
                    )
                ))
        }
    })
    ...
    label_5 <- reactive({
        ...
        try(
            paste0( job_5_data()[1], "\n",
                    job_5_data()[3], " - ", job_5_data()[4], " Bulanan", "\n",
                    job_5_data()[6], " Popularity", " | ", job_5_data()[5], " Kekosongan"),
            TRUE
        )
    })
    ...
    template <- reactive({
        if( nrow(visNode()) == 1 ) {
            ...
            png::readPNG("./www/pathImage_10.png")
        } else if ( nrow(visNode()) == 3 ) {
            png::readPNG("./www/pathImage_10.png")
        } else if ( nrow(visNode()) == 4 ) {
            png::readPNG("./www/pathImage_10.png")
    ...
    plotInput <- reactive({
        ...
        if(input$returnpdf){
            ...
            pdf("./www/MYCareer_Report.pdf", width=as.numeric(8), height=as.numeric(11))
    ...
    output$pdflink <- downloadHandler(
        ...
        content <- function(file) {
            ...
            file.copy("./www/MyCareer_Report.pdf", file)
        })
